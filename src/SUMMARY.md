# Summary

[Introduction](README.md)
- [android development](android_development/android_development.md)
    - [android build flow on debian 12](android_development/android_build_flow_on_debian_12.md)
    - [expo not working on phone](android_development/expo_not_working_on_phone.md)
    - [reactnative with expo](android_development/reactnative_with_expo.md)
- [debian 12 post installation](debian_12_post_installation.md)
    - [add source list](debian_12_post_installation/add_sources_list.md)
    - [app indicator](debian_12_post_installation/app_indicator.md)
    - [desktop icons ng](debian_12_post_installation/desktop_icons_ng.md)
    - [graphics card driver switching](debian_12_post_installation/graphics_card_driver_switching.md)
    - [hiding grub bootloader](debian_12_post_installation/hiding_grub_bootloader.md)
    - [kernel updating](debian_12_post_installation/kernel_updating.md)
    - [mysql installation for xampp](debian_12_post_installation/mysql_installation_for_xampp.md)
    - [night light issues](debian_12_post_installation/night_light_issue.md)
    - [nvidia driver installation](debian_12_post_installation/nvidia_driver_installation.md)
    - [OBS Studio small UI issue](debian_12_post_installation/obs_scaling_issue.md)
    - [shortcut customization](debian_12_post_installation/shortcut_customizations.md)
    - [unnecessary apps](debian_12_post_installation/unnecessary_apps.md)
    - [XDM configurations](debian_12_post_installation/XDM_configurations.md)
- [development issues](development_issues.md)
    - [pyenv installation](development_issues/pyenv_installation.md)
- [hardware control](hardware_control.md)
	- [CPU temperature](hardware_control/CPU_temperature.md)
    - [command line system monitoring](hardware_control/command_line_system_monitoring.md)
- [Latex usage](latex_usage.md)
    - [latex installation](latex_usage/latex_installation.md)
- [network issues](network_issues.md)
    - [network issues](network_issues/network_issues.md)
    - [apt network issues](network_issues/apt_network_issues.md)
    - [Gloabl network proxy](network_issues/global_network_proxy.md)
    - [CURL proxy](network_issues/CURL_proxy.md)
    - [proxy in ubuntu](network_issues/proxy_in_ubuntu.md)
    - [docker proxy](network_issues/docker_proxy.md)
    - [Telegram Proxy](network_issues/Telegram_MTProxy.md)
- [personalize](personalize.md)
    - [changing system name](personalize/changing_system_name.md)
    - [small text issue](personalize/small_text_issue.md)
    - [desktop icons for applications](personalize/desktop_icons_for_applications.md)
    - [nautilus places](personalize/nautilus_places.md)
    - [Terminal customization](personalize/terminal_customization.md)
    - [Timezone settings](personalize/timezone_settings.md)
- [remote server connection](remote_server_connection/remote_server_connection.md)
    - [SCP file transportation](remote_server_connection/SCP_file_transportation.md)
- [SSH Tutorials](ssh/ssh.md)
    - [Adding a new ssh key](ssh/add_key.md)
    - [Checking Passphrase](ssh/check_passphrase.md)
    - [Generating a new ssh key](ssh/generate_key.md)
    - [SSH passphrase for logging in](ssh/ssh_passphrase_for_login.md)
- [Virtual Machine](vm/vm.md)
	- [virt-manager](vm/virt_manager.md)
- [tutorials](tutorials/tutorials.md)
	- [command line shortcuts](tutorials/command_line_shortcuts.md)
- [web development issues](web_development_issues.md)
    - [deploy Laravel on nginx](web_development_issues/deploy_laravel_on_nginx.md)
    - [postgres autoincrement sequence](web_development_issues/postgres_autoincrement_sequence.md)
    - [SSL configuration for nginx](web_development_issues/SSL_configuration_for_nginx.md)
    - [git authorization git ssh or API token](web_development_issues/git_authorization_git_ssh_or_API_token.md)
    - [git ignore file mode](web_development_issues/git_ignore_file_mode.md)
    - [import sql from commandline](web_development_issues/import_sql_from_commandline.md)
    - [laravel broadcasting instructions](web_development_issues/laravel_broadcasting_instructions.md)
    - [web development issues](web_development_issues/imprt_sql_from_commandline.md)
    - [point domain to VPS manually](web_development_issues/point_domain_to_VPS_manually.md)
    - [NextJS deploy](web_development_issues/nextjs_deploy.md)
    - [mysql installation](web_development_issues/mysql_installation.md)
    - [docker installation and configuration](web_development_issues/docker_installation_and_configuration.md)
    - [docker important files](web_development_issues/docker_important_files.md)
    - [docker container up](web_development_issues/docker_container_up.md)
    - [xampp on ubuntu](web_development_issues/xampp_on_ubuntu.md)
- [accessing android interface](accessing_android_interface.md)
- [add user to group](add_user_to_group.md)
- [adding or removing users](adding_or_removing_users.md)
- [apt repository manipulation](apt_repository_manipulation.md)
- [bash auto completion files usage](bash_auto_completion_files_usage.md)
- [clear terminal history](clear_terminal_history.md)
- [clipboard for ubuntu](clipboard_for_ubuntu.md)
- [compression in linux](compression_in_linux.md)
- [media formats problem](media_formats_problem.md)
- [mounting problems](mounting_problems.md)
- [debian suspend crashing](debian_suspend_crashing.md)
- [DNS setter for linux.md](DNS_setter_for_linux.md)
- [downloading with WGET](downloading_with_WGET.md)
- [find command](find_command.md)
- [FireWall tutorial](FireWall_tutorial.md)
- [font config not found](font_config_not_found.md)
- [Initialization for ubuntu 20.04](Initialization_for_ubuntu_20.04.md)
- [Laravel configuration on ubuntu 20.04 LTS](Laravel_configuration_on_ubuntu_20.04_LTS.md)
- [linux architecture](linux_architecture.md)
- [owner changing](owner_chaning.md)
- [phpmyadmin installation](phpmyadmin_installation.md)
- [postgres configs](psql_configs.md)
- [python virtual environment](python_virtual_environment.md)
- [rustup docs opens telegram](rustup_docs_opens_telegram.md)
- [scroll-jump issue fix in gnome GUI](scroll-jump_issue_fix_in_gnome_GUI.md)
- [shortcuts in linux](shortcuts_in_linux.md)
- [startup applications](startup_applications.md)
- [suspend or hibernate](suspend_or_hibernate.md)
- [symlink issues on NTFS](symlink_issue_on_NTFS.md)
- [there are stopped jobs](there_are_stopped_jobs.md)
- [tracking packages](tracking_packages.md)
- [ubuntu boot problems](ubuntu_boot_problems.md)
- [vscode default browser](vscode_default_browser.md)
- [x-ui3 installation with tls](x-ui3_installation_with_tls.md)
